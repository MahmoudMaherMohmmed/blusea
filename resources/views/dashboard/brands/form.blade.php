    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                @if($brand!=null)
                    <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{$brand->title}}</span>
                @endif
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ {{ $brand!=null ? __('brands.actions.edit') : __('brands.actions.create') }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" action="{{$brand!=null ? route('admin.brands.update', $brand->id) : route('admin.brands.store')}}" enctype="multipart/form-data" data-parsley-validate="">
                        @csrf
                        @if($brand!=null) @method('PUT') @endif
                        <div class="row row-sm">

                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('brands.attributes.title') }} <span class="tx-danger">*</span></label>
                                    <div class="example">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-title-{{ $localeCode }}">
                                                            <input class="form-control" name="title[{{ $localeCode }}]" placeholder="{{ __('brands.attributes.title') }}" value="{{old('title.' . $localeCode, $brand?->getTranslation('title', $localeCode))}}" type="text" required>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-description-{{ $localeCode }}">
                                                            <textarea class="form-control" name="description[{{ $localeCode }}]" placeholder="{{ __('brands.attributes.description') }}"
                                                            rows="5">{{old('description.' . $localeCode, $brand?->getTranslation('description', $localeCode))}}</textarea>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <span class="text-danger" style="font-size: 11px;">{{ __('brands.messages.image_dimensions') }}</span>
                                </div>
                            </div>

                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('brands.attributes.status') }} <span class="tx-danger">*</span></label>
                                    <select class="form-control select2-no-search" name="status" required="">
                                        @foreach(App\Enums\BrandStatusEnum::options() as $key=>$value)
                                            <option value="{{$key}}" {{$brand!=null && $brand->status->value==$key ? 'selected' : ''}}> {{ $value }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <div class="col-12"><button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit">{{ $brand!=null ? __('brands.actions.edit') : __('brands.actions.save') }}</button></div>
                        </div>
                    </form>
                </div>
            </div>
