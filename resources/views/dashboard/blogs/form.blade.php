...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" action="{{$blog!=null ? route('admin.blogs.update', $blog->id) : route('admin.blogs.store')}}" enctype="multipart/form-data" data-parsley-validate="">
                        @csrf
                        @if($blog!=null) @method('PUT') @endif
                        <div class="row row-sm">

                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('blogs.attributes.title') }} <span class="tx-danger">*</span></label>
                                    <div class="example">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-title-{{ $localeCode }}">
                                                            <input class="form-control" name="title[{{ $localeCode }}]" placeholder="{{ __('blogs.attributes.title') }}" value="{{old('title.' . $localeCode, $blog?->getTranslation('title', $localeCode))}}" type="text" required>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('blogs.attributes.description') }} <span class="tx-danger">*</span></label>
                                    <div class="example">
										<div class="panel panel-primary tabs-style-1">
                                            <div class=" tab-menu-heading">
                                                <div class="tabs-menu1">
                                                ...
                                                <div class="tab-content">
                                                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                        <div class="tab-pane {{$loop->first ? 'active' : ''}}" id="tab-description-{{ $localeCode }}">
                                                            ...
                                                            rows="5" required>{{old('description.' . $localeCode, $blog?->getTranslation('description', $localeCode))}}</textarea>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2-no-search" name="status" required="">
                                        @foreach(App\Enums\BlogStatusEnum::options() as $key=>$value)
                                            <option value="{{$key}}" {{$blog!=null && $blog->status->value==$key ? 'selected' : ''}}> {{ $value }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <div class="col-12"><button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit">{{ $blog!=null ? __('blogs.actions.edit') : __('blogs.actions.save') }}</button></div>
                        </div>
                    </form>
                </div>
            </div>
