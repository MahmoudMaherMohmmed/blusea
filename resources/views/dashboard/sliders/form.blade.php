...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="post" action="{{$slider!=null ? route('admin.sliders.update', $slider->id) : route('admin.sliders.store')}}" enctype="multipart/form-data" data-parsley-validate="">
                        @csrf
                        @if($slider!=null) @method('PUT') @endif
                        <div class="row row-sm">
                                        ...
                                        <div class="panel panel-primary tabs-style-1">
                                            <div class=" tab-menu-heading">
                                                <div class="tabs-menu1">
                                                    ...
                                                    <ul class="nav panel-tabs main-nav-line">
                                                        @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                                            <li class="nav-item"><a href="#tab-subtitle-{{ $localeCode }}" class="nav-link {{$loop->first ? 'active' : ''}}" data-toggle="tab">{{ $properties['native'] }}</a></li>
                                                        @endforeach
                                                    </ul>
                                                </div>
                                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <label class="form-label">{{ __('sliders.attributes.status') }} <span class="tx-danger">*</span></label>
                                    <select class="form-control select2-no-search" name="status" require="">
                                        @foreach(App\Enums\SliderStatusEnum::options() as $key=>$value)
                                            <option value="{{$key}}" {{$slider!=null && $slider->status->value==$key ? 'selected' : ''}}> {{ $value }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            <div class="col-12"><button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit">{{ $slider!=null ? __('sliders.actions.edit') : __('sliders.actions.save') }}</button></div>
                        </div>
                    </form>
                </div>
            </div>
