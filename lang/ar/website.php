<?php

return [
    //Menu
    'home' => 'الرئيسية',
    'about_us' => 'عنا',
    'products' => 'المنتجات',
    'categories' => 'أقسام المنتجات',
    'brands' => 'العلامات التجارية',
    'blogs' => 'الأخبار',
    'contact' => 'تواصل معنا',

    //Home
    'special_products' => 'المنتجات المميزة',
    'special_products_subtitle' => 'مجموعتنا الحصرية: حيث يلتقي الابتكار بالأناقة.',
    'best_sellers' => 'الأكثر مبيعًا',
    'best_sellers_subtitle' => 'الأكثر تقييمًا، والأكثر حبًا، ومخصصة لك. تسوق أفضل المنتجات لدينا اليوم.',
    'our_blogs' => 'أخبارنا',
    'our_blogs_subtitle' => 'من أفكارنا إليك - اكتشف، تعلّم، وتطوّر.',
    'partners' => 'شركاؤنا',
    'partners_subtitle' => 'نبني النجاح معًا - تعرف على شركائنا الموثوقين.',
    'subscribe' => 'اشترك في النشرة الإخبارية',
    'subscribe_subtitle' => 'لا تفوت شيئًا - اشترك للحصول على التحديثات التي تهمك.',
    'write_your_email' => 'اكتب بريدك الإلكتروني...',

    //Products
    'latest_products' => 'أحدث المنتجات',
    'product_description' => 'وصف المنتج',
    'availability' => 'التوافر',
    'in_stock' => 'متوفر',
    'search_products' => 'ابحث عن المنتجات...',

    //Blog
    'recent_post' => 'الأخبار الحالية',
    'product_categories' => 'أقسام المنتجات',
    'read_more' => 'المزيد',

    //Contact
    'contact_title' => 'ابقي على تواصل',
    'contact_subtitle' => 'سواء كانت تعليقات، استفسارات، أو تحية ودية، نحن دائمًا جاهزون للتواصل. تواصل معنا الآن وسوف نقوم بالرد عليك فى اقرب وقا.',
    'phone' => 'رقم الهاتف',
    'email' => 'البريد الإلكتروني',
    'location' => 'العنوان',
    'view_on_google_map' => 'عرض على خريطة جوجل',
    'contact_your_name' => 'الاسم',
    'contact_name' => 'الاسم',
    'contact_your_email' => 'بريدك الإلكتروني',
    'contact_email' => 'البريد الإلكتروني',
    'contact_subject' => 'عنوان الرسالة',
    'contact_message' => 'الرسالة',
    'contact_send_message' => 'إرسال',
    'see_more' => 'عرض الكل',
    'what_we_do' => 'ماذا يمكننا أن نفعل ؟',
    'reads_our_latest' => 'أقرا أخر',
    'news_blog' => 'المقالات والأخبار',
    'powerful_features_of' => 'الميزات القوية ل',
    'logistics' => 'للخدمات اللوجستية',

    //Footer
    'useful_links' => 'روابط مفيدة',

    //Currency
    'currency' => '$',
];
